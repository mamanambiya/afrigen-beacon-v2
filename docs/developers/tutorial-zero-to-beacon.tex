...
\DefineVerbatimEnvironment{verbatim}{Verbatim}{breaklines,breakanywhere,fontsize=\small,frame=leftline,framerule=3pt,rulecolor=\color{codebg},framesep=6pt}
\usepackage{bookmark}
...
\textbf{Every command on this page is written for a POSIX shell} ---
bash or zsh, as on macOS and Linux. Two things break in PowerShell:

\begin{itemize}
\tightlist
\item
  \textbf{Line continuation.} These pages end a wrapped command with a
  backslash. PowerShell uses a backtick (\texttt{\textasciigrave{}})
  instead, so it treats the backslash line as a complete command and
  fails. Either join the command onto one line, or swap the trailing
  \texttt{\textbackslash{}} for \texttt{\textasciigrave{}}.
\item
  \textbf{\texttt{sed} does not exist.} Step 2 uses it to edit one line
  of a config file. Edit that file in a text editor instead.
\end{itemize}

\textbf{The smoother path is WSL2.} Docker Desktop on Windows already
runs a WSL2 backend, so opening a WSL shell costs nothing and every
command here then works exactly as written, with no translation. Run
\texttt{wsl} and clone the repository inside the Linux filesystem ---
not under \texttt{/mnt/c}, where Docker bind mounts are slow.

If you would rather stay in PowerShell, the joining rule above is the
whole translation. For example, step 6's check becomes one line:
...
\textbf{Use the HTTPS URL above unless you have already set up an SSH
key with GitHub.} The repository is public, so HTTPS needs no account at
all. The SSH form,
\texttt{git@github.com:AfriGen-D/variant-checker-beacon.git},
authenticates even for public repositories, and without a key on your
GitHub account it fails with
...
which reads as though the repository is missing or private. It is
neither --- that is an authentication failure wearing a misleading
message.

\textbf{The directory is \texttt{variant-checker-beacon}.} The
repository was renamed from \texttt{afrigen-beacon-v2};
\texttt{git\ clone} names the directory after the repository, not after
the old name.
...
On GNU/Linux, without the empty argument after \texttt{-i}:

\begin{verbatim}
sed -i 's/^SECURE_SSL_REDIRECT=True/SECURE_SSL_REDIRECT=False/' .env.boolean
\end{verbatim}

On Windows PowerShell, \texttt{sed} does not exist:
...
Only remove a network the inspect showed as \textbf{0 attached}, and
only one that belongs to something of yours.
...
\textbf{Your position will be different, and that is not cosmetic.} The
loader picks each position with
\texttt{random.randint(1000000,\ 50000000)}, so every install holds a
different 100 variants. A position copied from this page will almost
certainly not exist in your database, and the beacon will correctly
answer \texttt{exists:\ false} --- which looks exactly like a broken
beacon.
...
\textbf{\texttt{numTotalResults} really is 0 here, and that is not a
contradiction.} On this path the counter reports matching
\emph{datasets}, not matching variants. Read \texttt{exists} and never
infer absence from the count --- see \url{api-contract.md}.

Note the double quotes around the URL. \texttt{\$POS} does not expand
...
\textbf{The body is in double quotes, and the inner quotes are escaped.}
JSON needs double quotes and the shell needs \texttt{\$POS} to expand,
so single quotes will not do here. Pasting a literal
\texttt{\textless{}POS\textgreater{}} sends invalid JSON and the server
answers \texttt{500}, which tells you nothing about what you did wrong.

It must return exactly what the GET in Step 8 returned --- if POST and
GET ever disagree, one of them is wrong.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{/api/} & Beacon info \\
\texttt{/api/datasets} & What is loaded \\
\texttt{/api/entry\_types} & Supported entry types \\
\texttt{/api/map} & Endpoint map \\
\texttt{/api/individuals?sex=MALE} & A filtered entity query \\
\end{longtable}
}
